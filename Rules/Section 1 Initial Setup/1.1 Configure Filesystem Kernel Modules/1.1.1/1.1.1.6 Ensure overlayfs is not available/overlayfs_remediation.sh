#!/usr/bin/env bash
# CIS 1.1.1.6 Remediation Script - Disable overlayfs Kernel Module (Simplified)
...
# 1. Check if overlayfs exists in kernel modules
if find /lib/modules/$(uname -r) -type f -path "*/fs/${mod_name}/*.ko*" >/dev/null 2>&1; then
    ...
    # 3. Ensure /etc/modprobe.d/overlayfs.conf exists with disable rules
