...
###############################################################################
#
# Bash Remediation Script for CIS Ubuntu 24.04 Level 1 Server Benchmark [CUSTOMIZED]
#
# Profile Description:
# This baseline aligns to the Center for Internet Security Ubuntu 24.04 LTS Benchmark, v1.0.0, released 08-26-2024.
#
...
# $ oscap xccdf generate fix --profile xccdf_org.ssgproject.content_profile_cis_level1_server_customized --fix-type bash xccdf-file.xml
#
...
# Remediation is applicable only in certain platforms
if dpkg-query --show --showformat='${db:Status-Status}' 'gdm3' 2>/dev/null | grep -q '^installed$' && { [ ! -f /.dockerenv ] && [ ! -f /run/.containerenv ]; }; then
...
find  /var/log/  -type f ! -user 0 -regextype posix-extended -regex '.*\.journal(~)?$' -exec chown -L 0 {} \;
...
find -L /var/log/ -maxdepth 1 -type f ! -user 0 -regextype posix-extended -regex '.*lastlog(\.[^\/]+)?$' -exec chown -L 0 {} \;
...
find -L /var/log/ -maxdepth 1 -type f ! -user 0 -regextype posix-extended -regex '.*(b|w)tmp((\.|-)[^\/]+)?$' -exec chown -L 0 {} \;
...
find -L /var/log/apt/ -maxdepth 1 -type f ! -user 0 -regextype posix-extended -regex '^.*$' -exec chown -L 0 {} \;
...
find  /var/log/gdm/  -type f ! -user 0 -regextype posix-extended -regex '.*' -exec chown -L 0 {} \;
...
find  /var/log/gdm3/  -type f ! -user 0 -regextype posix-extended -regex '.*' -exec chown -L 0 {} \;
